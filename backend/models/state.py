""" Pydantic Validation For OpenSky Response """
...
class StateVector(BaseModel):
    """ OpenSky StateVector Validation """
    icao24: str
    ...
    def from_list(cls, data: List):
        """ Converting Data From List to Pydantic Type """
        return cls(
            icao24=data[0],
            callsign=data[1],
...
class States(BaseModel):
    ...
    def parse_states(self, values):
        """ Parsing States """
        if "states" in values and isinstance(values["states"], list):
            values["states"] = [
                StateVector.from_list(state) for state in values["states"]
            ]
        return values
